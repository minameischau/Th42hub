...
/**
 * Retrieves the markup for related hubposts
 *
 * @param  array  $args The element setting
 * @return string       The final markup
 */
function auxhp_get_hubpost_related_posts( $args = array() ){
    ...
    $defaults = array(
        'title'                       => auxin_get_option('hubpost_related_posts_label', __( 'Related Projects', 'auxin-hubpost' ) ),
        'post_type'                   => 'hubpost',
        'taxonomy_name'               => 'hubpost-cat', // the taxonomy that we intent to display in post info
        'taxonomy'                    => array( 'hubpost-cat', 'hubpost-tag' ),
        'desktop_cnum'                => auxin_get_option('hubpost_related_posts_column_number', 3 ),
        'tablet_cnum'                 => 2,
        'phone_cnum'                  => 1,
        ...
        'image_aspect_ratio'          => auxin_get_option('hubpost_related_image_aspect_ratio', 0.56 ),
        'preview_mode'                => auxin_get_option( 'hubpost_related_posts_preview_mode', 'grid' ),
        ...
        'display_like'                => true,
        'extra_classes'               => auxin_get_option('hubpost_related_posts_align_center', false ) ? 'aux-text-align-center': '',
    ...
    if( 'default' == $snap_related_item = auxin_get_post_meta( $post, '_related_posts_snap_items', 'default' ) ) {
        $snap_related_item = auxin_get_option( 'hubpost_related_posts_snap_items', false );
    }
    ...
    if( 'default' == $full_width = auxin_get_post_meta( $post, '_related_posts_full_width', 'default' ) ) {
        $full_width = auxin_get_option( 'hubpost_related_posts_full_width', false );
    }
