<?php
    ...
    $land_item_class_name = auxin_get_option( 'show_hubpost_land_side_entry_box_colors' ) ? ' aux-item-land' : '';

?>
                        <article <?php post_class( 'aux-single-hubpost-wrapper aux-hubpost-land' . $land_item_class_name ); ?> >
                            <?php if ( $has_attach ) { ?>
                                    ...
                                    <div class="entry-media">
                                        <div class="aux-media-frame aux-media-image">
                                            <a href="<?php echo !empty( $the_link ) ? $the_link : get_permalink(); ?>">
                                                <img src="<?php  echo AUXIN_URL . 'images/welcome/image-frame.svg'; ?>" class="auxin-attachment auxin-featured-image attachment-1024x1024" alt="hubpost default image" >
                                            </a>
                                        </div>
                                    </div>
                            <?php   }
                            ...
                            <div class="aux-land-side">
                                ...
                                <footer class="entry-meta">
                                    <div class="hubpost-tax">
                                        <span class="entry-tax">
                                        <?php // the_category(' '); we can use this template tag, but customizable way is needed! ?>
                                        <?php $tax_name = 'hubpost-cat';
                                              $cat_terms = wp_get_post_terms( $post->ID, $tax_name );
                                              if( $cat_terms = wp_get_post_terms( $post->ID, $tax_name ) ){
                                                  foreach( $cat_terms as $term ){
                                                      echo '<a href="'. get_term_link( $term->slug, $tax_name ) .'" title="'.__("View all posts in ", 'auxin-hubpost'). $term->name .'" rel="category" >'. $term->name .'</a>';
                                                  }
                                              }
                                        ?>
                                        </span>
                                        <?php if( ! empty($cat_terms) ){
                                            edit_post_link(__("Edit", 'auxin-hubpost'), '<i> | </i>', '');
                                            } else {
                                                edit_post_link(__("Edit", 'auxin-hubpost'), '', '');
                                            }
                                        ?>
                                    </div>
                                    <?php
                                    if( function_exists('wp_ulike') && auxin_get_option( 'show_hubpost_archive_like_button' ) ) { ?>
                                        <div class="comments-iconic">
                                            <?php
                                                wp_ulike( 'get', array( 'style' => 'wpulike-heart', 'button_type' => 'image', 'wrapper_class' => 'aux-wpulike aux-wpulike-hubpost-widget' ) );
                                            ?>
                                        </div>
                                    <?php  } ?>
