<?php
    ...
    $post_vars = auxhp_get_hubpost_config( $post, array( 
        'request_from' => 'single' ,
        'media_size'   => auxin_get_option( 'hubpost_single_image_size', '' ),
    ) );
    extract( $post_vars );
?>
...
<article id="post-<?php the_ID(); ?>" <?php post_class( $post_class ); ?> role="article" >
        ...
        if( $show_side ) {
            ...
            // Meta data options -----------------------------
            if( $display_cat ) {
                // get hubpost categories
                $tax_name = 'hubpost-cat';
        ...
        ?>
        <div class="entry-side aux-sticky-position <?php echo esc_attr( $header_class );?>" <?php echo $header_styles ;?>  data-boundry-target=".content .hubpost.hentry" data-sticky-move-method="after" data-boundaries="true" data-use-transform="true" data-sticky-margin="<?php echo $sticky_header_height; ?>">
        <?php } else { ?>
        ...
        <?php
            ...
            if( $_overview || $show_actions ) {
                ...
                if( $_overview ){
                    ...
                    do_action('auxin_single_hubpost_overview', $_overview, $show_like_btn, $show_share_btn );
                    echo '</div>';
                }
                if( $show_actions ) {
                    echo '<div class="entry-actions">';
                    do_action('auxin_single_hubpost_actions', $show_like_btn, $show_share_btn );
                    echo '</div>';
                }
                echo '</div>';
            }
        ...
        <?php
            if( $is_side_meta_set ){ // start of displaying condition
                ...
                if( $display_cat ){
                    printf( '<dt>%s</dt><dd><span class="entry-tax">', __( 'Categories', 'auxin-hubpost' ) );
                    foreach( $cat_terms as $term ){
                         echo '<a href="'. get_term_link( $term->slug, $tax_name ) .'" title="'.esc_attr__("View all posts in ", 'auxin-hubpost'). $term->name .'" rel="category" >'. $term->name .'</a>';
                    }
                    echo '</span></dd>';
                }

                if( $display_tag ) {
                    printf( '<dt>%s</dt><dd><span class="entry-tax">', __( 'Tags', 'auxin-hubpost' ) );
                    foreach( $tag_terms as $term ){
                         echo '<a href="'. get_term_link( $term->slug, $tax_name_tag ) .'" title="'. esc_attr__("View all posts in ", 'auxin-hubpost'). $term->name .'" rel="category" >'. $term->name .'</a>';
                    }
                    echo '</span></dd>';
                }
                ...
                if( $display_lunch ) { ?>
                    <a href="<?php echo $lunch_btn_url; ?>" class="aux-button aux-cta-button aux-black aux-medium aux-curve">
                        ...
                        <span class="aux-text"><?php echo auxin_get_option( 'hubpost_metadata_launch_label' ); ?></span>
                    </a><?php
                }
        ...
        wp_link_pages( array( 'before' => '<div class="page-links"><span>' . __( 'Pages:', 'auxin-hubpost') .'</span>', 'after' => '</div>' ) );

        // get next/prev hubpost buttons
        if( 'default' == $display_next_pre = auxin_get_post_meta( $post, '_show_next_prev_nav', 'default' ) ){
            $display_next_pre = auxin_get_option( 'show_hubpost_single_next_prev_nav', false );
        }
        ...
        if( auxin_is_true( $display_next_pre ) ) {
            if( 'default' == $next_prev_skin = auxin_get_post_meta( $post, '_next_prev_nav_skin', 'default' ) ){
                $next_prev_skin = auxin_get_option( 'hubpost_single_next_prev_nav_skin', false );
            }

            $next_label = auxin_get_option( 'hubpost_single_next_nav_label', '' );
            $prev_label = auxin_get_option( 'hubpost_single_prev_nav_label', '' );

            $args = array(
                'prev_text'      => __( esc_attr( $prev_label ), 'auxin-hubpost' ),
                'next_text'      => __( esc_attr( $next_label ), 'auxin-hubpost' ),
            ...
            if ( 'classic' == $next_prev_skin ) {
                $args['prev_text'] = __( 'Prev', 'auxin-hubpost' ) ;
                $args['next_text'] = __( 'Next ', 'auxin-hubpost' ) ;
            }
