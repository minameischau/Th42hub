<?php /* Loops through all hubpost posts */


// the page number
$paged            = max( 1, get_query_var('paged'), get_query_var('page') );
// get template type id
$template_type_id = auxin_get_option( 'hubpost_index_template_type', 'grid-1' );
// get template type
...
if( in_array( $template_type, array('grid', 'masonry', 'tiles') ) ){
    ...
    $args = array(
        ...
        'order'                       => 'desc',
        'num'                         => auxin_get_option( 'hubpost_archive_items_perpage' ),
        'display_like'                => auxin_get_option( 'show_hubpost_archive_like_button' ),
        'deeplink'                    => false,
        'item_style'                  => auxin_get_option( 'hubpost_archive_grid_item_type' ),
        'paginate'                    => false,
        'perpage'                     => false,
        'show_filters'                => false,
        'show_title'                  => true,
        'show_info'                   => true,
        'image_aspect_ratio'          => auxin_get_option( 'hubpost_image_aspect_ratio'),
        'space'                       => auxin_get_option( 'hubpost_archive_grid_space'),
        'desktop_cnum'                => auxin_get_option( 'hubpost_archive_column_number' ),
        'tablet_cnum'                 => auxin_get_option( 'hubpost_archive_column_number_tablet' ),
        'phone_cnum'                  => auxin_get_option( 'hubpost_archive_column_number_mobile' ),
    ...
    if( function_exists( 'auxin_widget_recent_hubposts_grid_callback' ) ){
        // get the shortcode base hubpost page
...
// if it is normal hubpost archive loop
} else {
    global $query_string;

    $pere_page = auxin_get_option( 'hubpost_archive_items_perpage', 12 );
...
// if it is not a shortcode base hubpost page
if( true === $result ){

    while ( have_posts() ) : the_post();

        include 'entry/hubpost-land.php';

    endwhile; // end of the loop.
// if it is a shortcode base hubpost page
} elseif( ! empty( $result ) ){
    echo $result;
