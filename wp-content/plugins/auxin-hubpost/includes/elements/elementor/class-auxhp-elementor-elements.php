<?php
namespace Auxin\Plugin\Hubpost\Elementor;
...
final class Elements {
    ...
    */
    public function init() {
        ...
        $this->dir_path = AUXHP_INC_DIR . '/elements/elementor';
    ...
    public function hooks() {
        ...
        // Register Elementor for hubpost post type
        add_action( 'auxin_plugin_updated',  array( $this, 'aux_add_hubpost_support' ) );

        // let Elementor pro override single hubpost template
    ...
    public function register_widgets( $widgets ) {
        $widgets['310'] = array(
            'file'  => $this->dir_path . '/recent-hubposts-grid.php',
            'class' => __NAMESPACE__ . '\Elements\Recent_Hubposts_Grid'
        );

        $widgets['320'] = array(
            'file'  => $this->dir_path . '/recent-hubposts-masonry.php',
            'class' => __NAMESPACE__ . '\Elements\Recent_Hubposts_Masonry'
        );

        $widgets['330'] = array(
            'file'  => $this->dir_path . '/recent-hubposts-tile.php',
            'class' => __NAMESPACE__ . '\Elements\Recent_Hubposts_Tile'
        );

        $widgets['340'] = array(
            'file'  => $this->dir_path . '/recent-hubposts-tile-carousel.php',
            'class' => __NAMESPACE__ . '\Elements\Recent_Hubposts_Tile_Carousel_Carousel'
        );
        $widgets['350'] = array(
            'file'  => $this->dir_path . '/recent-hubposts-grid-carousel.php',
            'class' => __NAMESPACE__ . '\Elements\Recent_Hubposts_Grid_Carousel'
        );
     ...
     * @access public
     */
    public function aux_add_hubpost_support() {
        $post_types = get_option( 'elementor_cpt_support' );
        
        if( ! $post_types ) {
            $post_types = [ 'page', 'post', 'hubpost' ]; 
            update_option( 'elementor_cpt_support', $post_types ); 
        } else if( ! in_array( 'hubpost', $post_types ) ) {
            $post_types[] = 'hubpost'; //append to array
            update_option( 'elementor_cpt_support', $post_types ); 
        }
    ...
    /**
     * Override hubpost templates
     *
     * Let Elementor Pro override hubpost templates
     *
     * @access public
     */
    public function aux_elementor_pro_override_templates( $need_override_location ) {
        
        return ( ( ( is_single() && get_post_type() == 'hubpost' ) || is_post_type_archive( 'hubpost' ) ) || $need_override_location ) ;
    }
	...
	public function register_tag( $dynamic_tags ) {
        ...
        $tags = array(
			'aux-hubposts-url' => array(
                'file'  => AUXHP_INC_DIR . '/elements/elementor/dynamic-tags/hubposts-url.php',
				'class' => 'DynamicTags\Auxin_Hubposts_Url',
				'group' => 'URL',
				'title' => 'URL',
			)
        );
