 ...
 */
function auxhp_get_template_part( $slug, $name = '' ) {
    auxin_get_template_part( $slug, $name, AUXHP()->template_path() );
}
...
if ( ! function_exists('auxhp_get_hubpost_config') ) {

    function auxhp_get_hubpost_config( $post, $settings ) {
        ...
        if( 'default' == $side_pos = auxin_get_post_meta( $post, '_side_info_pos', 'default' ) ){
            ...
            $side_pos = auxin_get_option( 'hubpost_single_side_pos', 'right' );
        }

        // whether to show ro hide the hubpost metadata info
        if( 'default' == $has_side_meta = auxin_get_post_meta( $post, '_show_side_info_meta', 'default' ) ){
            $has_side_meta = auxin_get_option( 'hubpost_single_display_side_info_meta', '1' );
        }
        ...
        if( 'default' == $display_cat = auxin_get_post_meta( $post, '_side_info_dicplay_cat', 'default' ) ){
            $display_cat = auxin_get_option( 'hubpost_single_display_category', true );
        }
        $args['display_cat'] = auxin_is_true( $display_cat );

         // get display_tag
        if( 'default' == $display_tag = auxin_get_post_meta( $post, '_side_info_dicplay_tag', 'default' ) ){
            $display_tag = auxin_get_option( 'hubpost_single_display_tag', true );
        }
        ...
        if( 'default' == $_alignment = auxin_get_post_meta( $post, '_overview_info_alignment', 'default' ) ){
            $_alignment = auxin_get_option( 'hubposts_overview_info_alignment', true );
        }
        ...
        if ( 'bottom' != $side_pos ) {
            if( 'default' == $sticky_sidebar = auxin_get_post_meta( $post, '_sticky_sidebar', 'default' ) ){
                $sticky_sidebar = auxin_get_option( 'hubpost_single_sticky_sidebar', false );
            }
            // sticky sidebar
        ...
        // action buttons
        $args['show_share_btn'] = ( auxin_get_option( 'show_hubpost_single_share', true ) );
        $args['show_like_btn' ] = ( auxin_get_option( 'show_hubpost_single_like', true ) );
        $args['show_actions'  ] = ( auxin_get_option( 'show_hubpost_single_share_like_section', true ) );
